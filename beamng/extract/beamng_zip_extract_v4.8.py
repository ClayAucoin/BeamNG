#!/usr/bin/env python3
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.8.py" -r "D:\__BeamNG__\___mods___" --out-base-dir "C:\Users\Administrator\Dropbox\__BeamNG__\____directory-extract____\output"
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.8.py" -r "M:\__BeamNG__\___mods___" --out-base-dir "C:\Users\Administrator\Dropbox\__BeamNG__\____directory-extract____\output"
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.8.py" -r "C:\__BeamNG__\___mods___" --out-base-dir "C:\Users\Administrator\Dropbox\__BeamNG__\____directory-extract____\output"
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.8.py" -r "C:\Users\Administrator\AppData\Local\BeamNG\BeamNG.drive" --out-suffix "mods" --out-base-dir "C:\Users\Administrator\Dropbox\__BeamNG__\____directory-extract____\output"
...
def zip_internal_mtime_minmax(zf: ZipFile) -> Tuple[str, str]:
    """
    Returns (min_iso, max_iso) from ZipInfo.date_time across all non-directory entries.
    Zip timestamps are stored without timezone; we format them as naive local-like strings.
    """
    mn = None
    mx = None
    for zi in zf.infolist():
        ...
        if name.endswith("/"):
            continue
        try:
            dt = datetime(*zi.date_time)  # (Y,M,D,H,M,S) tuple
        except Exception:
            continue
    ...
    fmt = "%Y-%m-%d %H:%M:%S"
    return (mn.strftime(fmt) if mn else "", mx.strftime(fmt) if mx else "")
...
def main():
        ...
        if cached_extras is not None:
            ...
            try:
                with ZipFile(zp, "r") as zf:
                    min_m, max_m = zip_internal_mtime_minmax(zf)
                    if min_m:
                        row["zip_inner_mtime_min"] = min_m
                    if max_m:
                        row["zip_inner_mtime_max"] = max_m
    ...
    if _should_show_popup(auto=True, popup_flag=args.popup):
        ...
        if rootPath == "C:\\Users\\Administrator\\AppData\\Local\\BeamNG\\BeamNG.drive":
