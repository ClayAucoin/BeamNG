#!/usr/bin/env python3
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.6.py" -r "D:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.6.py" -r "M:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"

# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.6.py" -r "C:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"
# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.6.py" -r "C:\Users\Administrator\AppData\Local\BeamNG\BeamNG.drive\current\mods" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output" --out-suffix beamng_mods

# python "C:\Users\Administrator\projects\BeamNG\beamng\extract\beamng_zip_extract_v4.6.py" -r "R:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"

# python "beamng_zip_extract_v4.6.py" -r "D:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"
# python "beamng_zip_extract_v4.6.py" -r "M:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"
# python "beamng_zip_extract_v4.6.py" -r "C:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"
# python "beamng_zip_extract_v4.6.py" -r "R:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output"

# python beamng_zip_extract_v4.5.py -r "C:\__BeamNG__\___mods___" --out-base-dir "G:\My Drive\__BeamNG__\____directory-extract____\output" --out-suffix beamng_mods
...
import argparse, csv, os, json, re, hashlib, time, sys, sqlite3  # noqa: E401
...
def path_has_excluded_dir(path: str, exclude: set) -> bool:
...
def compute_output_path(
    ...
    # clean the suffix so it's filename-safe
    suffix = (out_suffix or "").strip()
    if suffix:
        suffix = re.sub(r"[^A-Za-z0-9._-]+", "_", suffix)
        return os.path.join(base, f"mods_index_on_{letter}__{suffix}.csv")
...
def is_under_any(root: str, names: List[str], path: str) -> bool:
    p = path.replace("\\", "/").lstrip("/")
    ...
    if parts[0].lower() != root.lower():
        return False
    child = parts[1].lower()
    return any(child == n.lower() for n in names)
...
def collect_roots_from_json_paths(
    ...
    for p in info_paths:
        ...
        if len(parts) == 3 and parts[2].lower() == "info.json":
            ...
            name = parts[1]
            nl = name.lower()
            if nl in NEVER_ROOT_NAMES:
                continue
            if top == "levels" and nl not in seen_maps:
                seen_maps.add(nl)
                maps.append(name)
            elif top == "vehicles" and nl not in seen_veh:
                seen_veh.add(nl)
    ...
    for p in app_paths:
        ...
        if (
            ...
            and parts[-1].lower() == "app.json"
        ):
            name = parts[3]
            nl = name.lower()
            if nl in NEVER_ROOT_NAMES:
                continue
            if nl not in seen_ui:
                seen_ui.add(nl)
                ui_apps.append(name)

    return ";".join(maps), ";".join(vehicles), ";".join(ui_apps)
...
def select_jsons(info_paths, app_paths, map_name, vehicle_name):
    ...
    map_names = split_names(map_name)
    vehicle_names = split_names(vehicle_name)

    had_levels = bool(levels)
    had_vehicles = bool(vehicles)

    if levels and map_names:
        levels = [p for p in levels if is_under_any("levels", map_names, p)]
...
def write_message_file(
    messages_dir: str, row_id: str, message: str, refresh_existing: bool
) -> bool:
    """
    Writes <row_id>.txt if:
      - file doesn't exist, OR
      - refresh_existing=True

    Returns True if a file was written/overwritten, False if skipped.
    """
    os.makedirs(messages_dir, exist_ok=True)
    fp = os.path.join(messages_dir, f"{row_id}.txt")

    if (not refresh_existing) and os.path.exists(fp):
        return False
...
def main():
    ...
    ap.add_argument(
        "--out-suffix",
        default="",
        help="Optional suffix for the output filename to prevent overwrites. "
            "Example: --out-suffix current_mods -> mods_index_on_C__current_mods.csv",
    )
    ap.add_argument(
        "--out-base-dir",
    ...
    ap.add_argument(
        "--refresh-existing",
        action="store_true",
        help="Overwrite message files even if they already exist. (default OFF)",
    )
    ap.add_argument(
        "--prune-missing",
        action="store_true",
        help="Delete message files in the messages folder that don't match any row_id found in this run. (default OFF)",
    )
    args = ap.parse_args()
    ...
    out_path = compute_output_path(args.root, args.output, args.out_base_dir, args.out_suffix)
        ...
        if cached_extras is not None:
            ...
            try:
                ...
                if isinstance(full_msg, str) and full_msg.strip():
                    ...
                    message_row_ids_seen.add(row["row_id"])
                    write_message_file(
                        messages_dir, row["row_id"], full_msg, args.refresh_existing
                    )
                ...
                if "zip_error" not in row:
                    extras = {k: v for k, v in row.items() if k not in FILE_INFO_COLS}
                    _cache_put_extras(conn, _abspath, _fsz, _mtime, extras)
                    conn.commit()
        ...
        if os.path.isdir(messages_dir):
            for fn in os.listdir(messages_dir):
                if not fn.lower().endswith(".txt"):
                    continue
                row_id = fn[:-4]  # strip ".txt"
                if row_id not in message_row_ids_seen:
                    try:
